...
- **MPRIS Integration:**
  - Expose `get_mpris_metadata` to retrieve active media player metadata (title, artist, album, artUrl, status, length, position) via playerctl MPRIS API.
  - Expose `update_mpris_metadata` to serialize and inject this player metadata into WebKit frame via window.onMetadataUpdate callback.
  - Import `subprocess` and `json`.
